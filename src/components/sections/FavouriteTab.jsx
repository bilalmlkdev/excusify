// src/components/sections/FavoritesTab.jsx
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <p
          className={`text-[10px] font-sans font-semibold uppercase tracking-widest mb-3 ${isDark ? "text-zinc-600" : "text-zinc-400"}`}
        >
        ...
        {favorites.length > 0 && (
          <button
            ...
            className={`text-xs font-sans px-3 py-1 rounded-lg border transition-all cursor-pointer -mt-3 ${clearCl}`}
          >
            clear all
          </button>
        )}
      ...
      {favorites.length === 0 ? (
        <p
          className={`text-xs font-sans ${isDark ? "text-zinc-600" : "text-zinc-400"}`}
        >
        ...
        <div className="space-y-2">
          {favorites.map((item, i) => (
            <div
              key={i}
              className={`text-xs font-sans rounded-xl border p-3 flex gap-2.5 ${entryBg}`}
            >
